...
const forwardCapture = function* (next) {
  this.url = '/capture.html';
  yield next;
};

module.exports = (router) => {
  router.all('/cgi-bin/**', bodyParser({ formLimit: '2mb' }));
  ...
  router.all('/user/:name', function () {
    const { name } = this.params;
    this.redirect(`../data.html?name=${name.replace(/\..*$/, '')}`);
  });
};
