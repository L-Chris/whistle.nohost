...
class EnvMgr {
  ...
  getEnvFromCookie(ctx) {
    const env = decodeURIComponentSafe(ctx.cookies.get(COOKIE_NAME));
    if (!env) {
      return '';
    }
  ...
  getEnvByHeader(ctx, fromComposer) {
    let name = decodeURIComponentSafe(ctx.get(WHISTLE_ENV_HEADER));
    if (!name) {
      return (fromComposer && this.getEnvFromCookie(ctx)) || '';
    }
